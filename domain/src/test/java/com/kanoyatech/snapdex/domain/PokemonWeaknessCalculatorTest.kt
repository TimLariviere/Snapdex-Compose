...
class PokemonWeaknessCalculatorTest {
    ...
    fun `Dual type has correct weaknesses after removing resistances`(types: List<PokemonType>, expected: List<PokemonType>) {
    ...
    companion object {
        @JvmStatic
        fun singleType() = listOf(
            Arguments.of(PokemonType.FIRE, listOf(PokemonType.GROUND, PokemonType.ROCK, PokemonType.WATER)),
            Arguments.of(PokemonType.WATER, listOf(PokemonType.ELECTRIC, PokemonType.GRASS)),
        )

        @JvmStatic
        fun dualType() = listOf(
            Arguments.of(listOf(PokemonType.FIRE, PokemonType.GRASS), listOf(PokemonType.FLYING, PokemonType.POISON, PokemonType.ROCK))
        )
    }
}
